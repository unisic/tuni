...
/// One terminal: where its shell was, how much room it had, and the key its
/// saved scrollback is filed under.
...
/// A restored workspace, and the histories its panes are waiting for.
pub struct Restored {
...
impl Snapshot {
    /// The workspace as it stands. `history` is asked for the key each pane's
    /// scrollback was saved under, and answers `None` for panes whose output
    /// was not saved — which is every pane when the setting is off.
    #[must_use]
    pub fn of(workspace: &Workspace, history: impl Fn(Id) -> Option<String>) -> Self {
    ...
    pub fn restore(&self) -> Restored {
        ...
        for saved in &self.projects {
            let tabs: Vec<Tab> = saved
                .tabs
                .iter()
                .filter_map(|tab| restore_tab(tab, &mut histories))
                .collect();
...
fn snap_tab(tab: &Tab, history: &impl Fn(Id) -> Option<String>) -> TabSnapshot {
...
fn restore_tab(saved: &TabSnapshot, histories: &mut HashMap<Id, String>) -> Option<Tab> {
...
mod tests {
    ...
    fn a_layout_comes_back_the_shape_it_was_saved_in() {
        ...
        let restored = Snapshot::of(&saved, |_| None).restore().workspace;
    ...
    fn a_history_key_follows_the_pane_it_was_saved_for() {
        ...
        let restored = Snapshot::of(&saved, |id| {
            (id == second).then(|| "key-for-the-second".to_owned())
        })
        .restore();
    ...
    fn a_snapshot_reads_back_out_of_its_own_json() {
        ...
        let snapshot = Snapshot::of(&saved, |_| None);
